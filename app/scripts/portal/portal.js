...
const toast = new Toastify({
  text:
    "<div class='toast-loading-text' style='width: 250px'><img src=" +
    logoWhite +
    " width='120' style='margin-bottom: 8px' /><p style='padding-bottom: 8px;'>Atualizando suas informações...</p>\n\n <b>NÃO SAIA DESSA PÁGINA,</b> <p>apenas aguarde, no máx. 5 min 🙏</p></div>",
  duration: -1,
  close: false,
  gravity: "bottom",
  position: "right",
...
async function getFichaAluno(fichaAlunoUrl, nomeDoCurso, anoDaGrade) {
  try {
    ...
    Toastify({
      text:
        "<div style='width: 228px; display: flex; align-items: end; margin-right: 12px;'><img style='margin-right: 16px;' width='32' height='32' src='" +
        errorSVG +
        "' /> Não foi possível salvar seus dados, recarregue a página e aguarde.</div>",
      duration: -1,
      close: true,
      gravity: "top",
      position: "right",
...
async function saveToLocalStorage(curso) {
  ...
  toastr.success(
    "Suas informações foram salvas! Disciplinas do curso do " +
      curso.curso +
      " para o usuário " +
      getEmailAluno() +
      ".",
